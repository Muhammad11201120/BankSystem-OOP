...
public:
	clsCurrency( enMode mode , string countryName , string currencyName , string currencySymbol , double currencyRate )
	{
		this->_mode = mode;
	...
	{
		return ( double ) ( Amount / this->_dblCurrencyRate );
	}
	double convertToAnotherCurrency( double Amount , clsCurrency Currency )
	{
		double amountInUSD = converToUsd( Amount );
		if ( Currency.currencySymbol() == "USD" )
		{
			return amountInUSD;
		}
		return ( double ) ( amountInUSD * Currency.currencyRate() );
	}
};